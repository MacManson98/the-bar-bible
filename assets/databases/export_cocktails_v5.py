...
Usage:
  python export_cocktails_v3.py [output_path]
...
# Categories:
#   Spirits      - Base alcohols, primary structure
#   Liqueurs     - Sweetened/fortified alcoholic modifiers (incl vermouth, amari, absinthe)
#   Citrus       - Fresh citrus used for juice or acid structure (incl whole citrus fruits)
#   Juices       - Non-citrus fruit juices and fruit purees
#   Sweeteners   - Sugar-based balancing ingredients
#   Mixers       - Non-alcoholic structural ingredients that change texture/body/length
#   Bitters      - Concentrated aromatic seasoning agents
#   Coffee       - Coffee-based ingredients
#   Other        - Condiments, spices, herbs, misc
...
CATEGORY_OVERRIDES = {
    ...
    'Tomato Juice':                   'Juices',
    'Passionfruit Puree':             'Juices',
    'White Peach Puree':              'Juices',

    # Spirits (fix miscategorised)
    "Gosling\u2019s Rum":         'Spirits',
    "Gosling's Rum":              'Spirits',
    'Ron Profundo Havana Club':   'Spirits',
    'Ron Smoky Havana Club':      'Spirits',

    # Mixers (structural/texture)
    'Ginger Ale':       'Mixers',
    'Champagne':        'Mixers',
    'Prosecco':         'Mixers',
    'Fresh Cream':      'Mixers',
    ...
    # Other (condiments, herbs, misc)
    'Basil Leaves':         'Other',
    'Gengibre Slice':       'Other',
    'Celery Salt':          'Other',
    'Pepper':               'Other',
    'Tabasco':              'Other',
...
def create_database(output_path):
    ...
    if not cocktails_data:
        print("No cocktails fetched!")
        return
    if not ingredients_data:
        print("No ingredients fetched!")
        return
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS cocktails (
        ...
        method TEXT NOT NULL, method_instructions TEXT, glass TEXT NOT NULL,
        difficulty INTEGER NOT NULL, base_spirit TEXT NOT NULL, ice TEXT,
        garnish TEXT, tags TEXT, history TEXT, tasting_notes TEXT, tiles_notes TEXT,
        image_path TEXT, category TEXT NOT NULL DEFAULT 'cocktail')''')

    cursor.execute('''CREATE TABLE IF NOT EXISTS ingredients (
        id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE,
        category TEXT NOT NULL DEFAULT 'Other')''')

    cursor.execute('''CREATE TABLE IF NOT EXISTS cocktail_ingredients (
        id INTEGER PRIMARY KEY AUTOINCREMENT, cocktail_id INTEGER NOT NULL,
        ingredient_id INTEGER NOT NULL, amount REAL NOT NULL, unit TEXT NOT NULL,
        prep_note TEXT, FOREIGN KEY (cocktail_id) REFERENCES cocktails(id),
        FOREIGN KEY (ingredient_id) REFERENCES ingredients(id))''')

    cursor.execute('''CREATE TABLE IF NOT EXISTS favorites (
        cocktail_id INTEGER PRIMARY KEY, favorited_at INTEGER NOT NULL,
    ...
    for c in cocktails_data:
        ...
        if not name:
            continue
        cursor.execute('''INSERT INTO cocktails (name, method, method_instructions, glass,
            difficulty, base_spirit, ice, garnish, tags, history, tasting_notes, tiles_notes, image_path, category)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (name, c.get('method', ''), c.get('method_instructions', ''),
             c.get('glass', ''), int(c.get('difficulty', 2) or 2), c.get('base_spirit', ''),
             c.get('ice', ''), c.get('garnish', ''), c.get('tags', ''),
             c.get('history', ''), c.get('tasting_notes', ''), c.get('tiles_notes', ''),
             generate_image_path(name), c.get('category', 'cocktail')))
    ...
    for rel in relationships:
        ...
        if cocktail_id and ingredient_id:
            ...
            cursor.execute('''INSERT INTO cocktail_ingredients
                (cocktail_id, ingredient_id, amount, unit, prep_note)
                VALUES (?, ?, ?, ?, ?)''',
    ...
    print(f"  {len(cocktail_map)} cocktails, {len(ingredient_map)} ingredients, {inserted} relationships")
